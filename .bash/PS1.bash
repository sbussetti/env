...
# =========================== #
#                             #
#    SCM working dir info     #
#                             #
# =========================== #

function parse_svn_dirty() {
  [[ $(svn status 2> /dev/null) ]] && printf "*"
}

function display_svn_info() {
  if [ "$SHOW_SVN" != true ] || ! svn info 2>&1 1>/dev/null; then return; fi
  echo -n " on ${SOLAR_CYAN}$(svn info 2>/dev/null | grep '^URL:' | grep -E -o '(tags|branches)/[^/]+|trunk' | grep -E -o '[^/]+$' | sed "s/$/$(parse_svn_dirty)/")${SOLAR_BASE1}"
}

function parse_hg_dirty() {
  [[ $(hg status 2> /dev/null) ]] && printf "*"
}

function display_hg_info() {
  if [ "$SHOW_HG" != true ] || ! hg branch 2>&1 1>/dev/null; then return; fi
  printf " on ${SOLAR_CYAN}$(hg branch 2> /dev/null | awk '{print $1}' | sed "s/$/$(parse_hg_dirty)/")${SOLAR_BASE1}"
}
...
function parse_git_dirty() {
  if [[ $(git status 2> /dev/null | tail -n1) != "$GIT_CLEAN" ]]; then
    printf "*";
  fi
}
